...
char save_ip[3][INET6_ADDRSTRLEN] = {0}; //jiawei
static int8_t i=0;//jiawei
//static int8_t j=0;//jiawei
/*==================================================================================================
Private macros
==================================================================================================*/
#define EOL                         "\r\n"
...
static int8_t SHELL_LED(uint8_t argc, char *argv []);//jiawei
...
const cmd_tbl_t aShellCommands[] =
{
	...
	{
		"LED", SHELL_CMD_MAX_ARGS, 0, SHELL_LED
/*#if SHELL_USE_HELP
        ,"webserver control led",
        "webserver control led\r\n"
        "LED 0/1/2/3  on/off - displays LED 0/1/2/3 on/off \r\n"
#endif /* SHELL_USE_HELP */
/*#if SHELL_USE_AUTO_COMPLETE
        ,NULL
#endif /* SHELL_USE_AUTO_COMPLETE */
...
//static instanceId_t mThrInstanceId = gInvalidInstanceId_c;
//static int leaderLedTimestamp = 0;
//extern void App_UpdateStateLeds(appDeviceState_t deviceState);

static int8_t SHELL_LED //jiawei
(
	uint8_t argc,
	char *argv[] //save pointer
)
{
	char *argv1[100];//save pointer
	char x[50]={0};
	SHELL_NEWLINE();
				  shell_write("not_enter");
				  SHELL_NEWLINE();
	/* LED <reqtype: 0/1/2/3> <reqcode: on/off> - displays led 0/1/2/3 on/off */
	if(!strcmp(argv[1], "0") && (!strcmp(argv[2], "ON")||!strcmp(argv[2], "OFF")))
	    {
					  SHELL_NEWLINE();
					  shell_write("enter_LED0");
					  SHELL_NEWLINE();
			if(!strcmp(argv[2], "ON"))
				{		  SHELL_NEWLINE();
				  	  	  shell_write("on");
	...
	//{
	 ...
	 // if(!strcmp(argv[1], "1") && (!strcmp(argv[2], "on") || !strcmp(argv[2], "off")))
			  else  if(!strcmp(argv[1], "1"))
	    {
				  	  	  	  SHELL_NEWLINE();
				  			  shell_write("half_enter");
				  			  SHELL_NEWLINE();
		  if(!strcmp(argv[2], "ON") || !strcmp(argv[2], "OFF"))
		  { 				  SHELL_NEWLINE();
		  	  	  	  	  	  shell_write("already_enter_1");
		  	  	  	  	  	  SHELL_NEWLINE();
		 	//j=0;
		 	argv1[1]="CON";
		 	argv1[2]="POST";
		 	//FLib_MemCpy(argv1[3], save_ip[j] , sizeof(ipAddr_t));
		 	strcpy(x, save_ip[0]);
		 						SHELL_NEWLINE();
		 						shell_write(save_ip[i]);
		 						SHELL_NEWLINE();
		 	argv1[3]=x;
		 	argv1[4]="/led";
		 	if(!strcmp(argv[2], "ON"))
		 		...
		 		{argv1[5]="off";}
		 			 		 			  		  ...
		 			 		 			  		  shell_write(argv1[5]);
		 			 		 			  			 SHELL_NEWLINE();
		 	SHELL_CoapSend(argc, argv1);}

		  	  	  	  	  	  SHELL_NEWLINE();
		  	  	  	  	  	  shell_write("end of already_enter");
		  	  	  	  	  	  SHELL_NEWLINE();
	    }
	 else if(!strcmp(argv[1], "2") && (!strcmp(argv[2], "ON")||!strcmp(argv[2], "OFF")))
		    {
		 	 	 	 	 	 	 	 	 SHELL_NEWLINE();
		 	 	 	 	 	 	 	 	 shell_write("already_enter_2");
		 	 	 	 	 	 	 	 	 SHELL_NEWLINE();
			 	//j=1;
			 	argv1[1]="CON";
			 	argv1[2]="POST";
	 ...
	 else if(!strcmp(argv[1], "3") && (!strcmp(argv[2], "ON")||!strcmp(argv[2], "OFF")))
		    {
		 	 	 	 	  SHELL_NEWLINE();
				 		  shell_write("already_enter_3");
				 		  SHELL_NEWLINE();
			 	//j=2;
			 	argv1[1]="CON";
			 	argv1[2]="POST";
...
)
{
    ...
    if (argc >= 3)
    {
        ...
        {
            /* Get destination address */
           // pton(AF_INET6,argv[3], &destAddr); //jiawei

        	pton(AF_INET6,argv[3], &destAddr); //jiawei  save_ip[3][INET6_ADDRSTRLEN]
        			  SHELL_NEWLINE();
        			  shell_write("enter_argv[3]");
        			  SHELL_NEWLINE();
        			  shell_write(argv[3]);
        			  SHELL_NEWLINE();
        ...
        while(remainingBytes != 0)
        {
            ...
            if(pTlvHeader->tlvType == gDiagnosticTlv_IPv6AddrList_c)
            {
                ...
                for(uint32_t iCount = 0; iCount < noOfAddr; iCount++)
                {
                    ...
                    if(IP_IsAddrIPv6(&ipAddr) && iCount == 0)//jiawei
                    {

                        ntop(AF_INET6, &ipAddr, addrStr, INET6_ADDRSTRLEN);
                        shell_write(addrStr);
                        SHELL_NEWLINE();
                        //FLib_MemCpy(save_ip[i], addrStr, sizeof(46));//jiawei
                        strcpy(save_ip[i],addrStr);
                        shell_write(save_ip[i]);
                        i=i+1;//jiawei
                        SHELL_NEWLINE();
                        if(i>=3)//jiawei
